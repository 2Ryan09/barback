...
export default {
  ...
  methods: {
    deleteRow (data) {
      axios.get('/api/offerBottles/' + data.id)
        .then((response) => {
          this.test = response.data;
          if (Object.keys(this.test).length === 0) {
            swal({
              title: 'Are you sure?',
              text: "A black hole will consume this offering!",
              type: 'warning',
              ...
              if (result.value) {
                axios.delete('/api/offerings/' + String(data.id))
              ...
              swal(
                'Uh Oh',
                'You cannot delete this offering, it has bottles attached!',
                'error'
              )
          }
        })
